...
이 문서는 `docs/evidence/k6/` 아래 local smoke artifact의 해석 기준을 정리합니다. 모든 항목은 local
evidence이며, 반복 가능한 throughput / latency / error-rate benchmark claim으로 사용하지 않습니다.
...
- public benchmark로 올리려면 hardware/JVM/DB/Redis 조건, dataset 규모, 반복 실행, p50/p95/p99,
...
- 반복 full mixed artifact는 `scripts/run-full-mixed-evidence.sh`로 수집합니다. 이 스크립트가 만드는
  `full-mixed-<timestamp>/` 디렉터리는 k6 summary/console, Prometheus before/after sample, sanitized
  metadata를 묶는 capture format입니다. 각 summary는 `scripts/validate-k6-full-mixed-summary.mjs`로
...
- capture format 자체는 benchmark 결과가 아닙니다. 결과를 공개하려면 run-to-run variance와 실행 환경을
  review한 뒤 `docs/PERF_RESULT.md`에 별도로 승격합니다.
