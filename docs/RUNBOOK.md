...
- 현재 invoice generation은 `organizationId + billingPeriod` unique constraint로 멱등성 시나리오를 검증합니다. Claim boundary: 분산 exactly-once 처리는 주장하지 않습니다.
...
- dashboard, tracing, SLO, alert rule 설정 파일과 캡처/쿼리 결과. Claim boundary: 아직 추가 측정 예정입니다.
